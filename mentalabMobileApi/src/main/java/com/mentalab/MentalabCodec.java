...
public class MentalabCodec {
     ...
     *
     * Incoming bytes from Bluetooth are converted to an immutable
     * Map of Queues of Numbers.
     * ExG channels are saved as single precision floating point
     * numbers (Float) in the unit of mVolt.
     ...
     */
   public static Map<String, Queue>
    decode(InputStream stream)
       ...
       while(loop_Count < 100){
           ...
           try {
               ...
               List<Float> voltageValueArray = parsePayloadData(pId, Arrays.copyOfRange(buffer, 0, buffer.length -5));
     ...
     * Encodes a command
     *
     * @throws InvalidCommandException when the command is
     * not recognized
     * @return byte[] encoded commands that can be sent to
     * the device
     */
    static byte[]
    encodeCommand(final String command)
